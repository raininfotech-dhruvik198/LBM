...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        # flux_guidance_scale is now fixed internally
        # flux_num_inference_steps is now fixed internally
        lbm_num_sampling_steps: int = Input(
            description="Number of inference steps for LBM model.", default=1, ge=1, le=4
        )
        # output_width and output_height are removed, defaults to original foreground size
        # seed is now fixed internally for reproducibility
    ) -> Path:
        """Run relighting prediction with a foreground image and a background generated by FLUX.1-Fill based on a prompt.
        Output image dimensions will match the original foreground image.
        FLUX parameters (guidance, steps, seed) are fixed internally for consistent output.
        """

        # Fixed internal parameters
        internal_seed = 42 # Fixed seed for reproducibility
        flux_internal_guidance_scale = 7.5
        flux_internal_num_inference_steps = 25

        torch.manual_seed(internal_seed)
        generator = torch.Generator(device=self.device).manual_seed(internal_seed)
        print(f"Using fixed internal seed: {internal_seed}")
        print(f"Using fixed FLUX guidance scale: {flux_internal_guidance_scale}")
        print(f"Using fixed FLUX inference steps: {flux_internal_num_inference_steps}")
        ...
        flux_output_image_pil = self.flux_fill_pipe(
            ...
            guidance_scale=flux_internal_guidance_scale,
            num_inference_steps=flux_internal_num_inference_steps,
            generator=generator,
